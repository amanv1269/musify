...
//     //   const existingSong = allSongs.find((s) => s._id === song.title);
//     // }
//     const urlparams = new URLSearchParams(window.location.search);
//     if (SongId && SongId !== "") {
...
//       <div>
//         <img src="https://img.freepik.com/free-vector/musical-melody-symbols-yellow-splotch_1308-64213.jpg" />
//       </div>
//  </div>
//     </div>
//   );
...
function AddEditSong() {
  ...
  const onAdd = async () => {
    try {
      ...
      const response = await axios.post(
        `${window.location.origin}/api/admin/add-song`,
        formData,
        {
          headers: {
  ...
  const onEdit = async () => {
    try {
      ...
      formData.append("_id", songId);
      const response = await axios.post(
        `${window.location.origin}/api/admin/edit-song`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
